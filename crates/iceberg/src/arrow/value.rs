            ...
            PrimitiveLiteralArrayBuilderInner::Decimal128(builder) => {
                ...
                let DataType::Decimal128(precision, scale) = array.data_type() else {
                    return Err(Error::new(
                        ...
                        format!("Expected Decimal128Array, got {:?}", array.data_type()),
                    ));
                };
                ...
                array.validate_decimal_precision(precision).map_err(|e| {
                    Error::new(
                        ErrorKind::DataInvalid,
                        format!(
                            "Failed to create Decimal128Array with precision {precision} and scale {scale}: {e}"
...
pub(crate) fn create_primitive_array_single_element(
    ...
    // Preserve this function's original error contract: before the builder
    // refactor an unsupported data type fell through to a single
    // `ErrorKind::Unexpected` arm, so map the builder's `FeatureUnsupported`
    // back to `Unexpected` with the same message rather than surfacing a new kind.
    let mut builder = PrimitiveLiteralArrayBuilder::try_new(data_type, 1).map_err(|_| {
        Error::new(
            ErrorKind::Unexpected,
...
mod test {
    ...
    #[test]
    fn test_primitive_literal_array_builder_rejects_decimal_exceeding_precision() {
        let target_type = DataType::Decimal128(2, 0);
        let value = PrimitiveLiteral::Int128(100);

        let mut builder = PrimitiveLiteralArrayBuilder::try_new(&target_type, 1).unwrap();
    ...
    #[test]
    fn test_create_decimal_array_single_element_rejects_value_exceeding_precision() {
        let target_type = DataType::Decimal128(2, 0);
        let value = PrimitiveLiteral::Int128(100);

        let err = create_primitive_array_single_element(&target_type, &Some(value)).unwrap_err();
        assert_eq!(err.kind(), ErrorKind::DataInvalid);
    }

    #[test]
    fn test_primitive_literal_array_builder_null_or_mismatch_appends_null() {
        let mut builder = PrimitiveLiteralArrayBuilder::try_new(&DataType::Int32, 3).unwrap();

        assert!(
            builder
                .append_or_null(Some(&PrimitiveLiteral::Int(42)))
                .unwrap()
        );
        assert!(!builder.append_or_null(None).unwrap());
        assert!(
            !builder
                .append_or_null(Some(&PrimitiveLiteral::String("forty-two".to_string())))
                .unwrap()
        );
