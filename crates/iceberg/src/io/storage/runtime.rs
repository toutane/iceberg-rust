...
impl RuntimeStorage {
    ...
    where
        ...
        T: Send + 'static,
    {
        // Spawned tasks must be 'static, so borrowed paths are owned at the runtime boundary.
...
impl Storage for RuntimeStorage {
    // `Storage` is a typetag trait; provide typetag's required bookkeeping
    // methods without registering this transient adapter for deserialization.
    #[doc(hidden)]
...
struct RuntimeFileWrite {
    // FileWrite methods take &mut self, but spawned IO tasks must own 'static values.
    // The mutex lets each spawned operation borrow the underlying writer after
    // it has moved into the wrapper.
